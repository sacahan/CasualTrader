...
- 部位風險分析（未實現損益、風險分數）
- 市場風險監控（透過 perplexity_mcp 搜尋市場風險、公司風險新聞）
- 壓力測試與情景分析
- 風險管理建議（停損、避險、部位調整）

## 🎯 perplexity_mcp 使用限制

⚠️ **重要**：perplexity_mcp 使用需要消耗點數，請遵守以下原則:
...
**步驟 1-3：風險數據收集** → casual_market_mcp + perplexity_mcp + tools
  1. 收集波動率、融資融券等風險數據
  2. 計算單一部位風險 → calculate_position_risk
  3. 分析組合集中度 → analyze_portfolio_concentration

**步驟 4-6：壓力測試與風險評級** → perplexity_mcp + tools
  4. 計算整體組合風險 → calculate_portfolio_risk
  5. 透過 perplexity_mcp 搜尋市場風險新聞、執行壓力測試 → perform_stress_test
