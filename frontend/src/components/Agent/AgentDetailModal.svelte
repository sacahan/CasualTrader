...
<Modal bind:open size="full" onclose={handleClose}>
  ...
  <div class="flex h-full flex-col">
    <div class="flex-1 overflow-hidden p-6">
      <!-- 概覽視圖：左圖表 + 右面板 -->
      <div class="grid h-full grid-cols-1 gap-6 lg:grid-cols-[minmax(0,1.2fr)_minmax(0,1fr)]">
        <!-- 左側：績效圖表 -->
        <div class="flex flex-col rounded-lg border border-gray-700 bg-gray-800 p-6">
          ...
          {#if performanceData && performanceData.length > 0}
            <div class="mt-8">
              <RiskMetricsCard metrics={performanceData[performanceData.length - 1]} {agentColor} />
            </div>
          {/if}
        </div>

        <!-- 右側：信息面板（可滾動） -->
        <div class="custom-scrollbar flex h-full flex-col gap-6 overflow-y-auto pr-1">
          <!-- KPI 指標區 -->
          <div class="space-y-3 rounded-lg border border-gray-700 bg-gray-800 p-6">
            <div class="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <p class="mt-1 text-lg font-bold text-white">{formatCurrency(currentCash)}</p>
              </div>
              <div>
