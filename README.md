...
台灣股價即時查詢 MCP Server，使用 **FastMCP 簡化架構**，提供透過 Model Context Protocol 存取台灣證券交易所即時股價資訊的功能。本專案整合 **AI Agent 動態策略系統**，實現智能交易決策和自動策略演化。
...
- 🔄 **四種 Agent 模式** - OBSERVATION, TRADING, REBALANCING, STRATEGY_REVIEW 智能循環
- 📈 **動態策略演化** - 基於投資效益自動調整和優化交易策略
- ⚡ **雙重驅動切換** - 時間調度 + 性能指標的智能模式切換
- 🛡️ **風險自適應** - 根據市場波動和組合表現動態調整風險參數
- 🧠 **知識圖譜累積** - 在觀察模式下持續建立市場關係和機會識別
- 📊 **性能追蹤分析** - 完整的績效歸因分析和基準比較系統
- 🎯 **策略變體測試** - 自動生成和驗證策略改進變體
